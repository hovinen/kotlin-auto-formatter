...
internal class WhileExpressionScanner(private val kotlinScanner: KotlinScanner) : NodeScanner {
    private val nodePattern = nodePattern {
        ...
        oneOrMore { anyNode() } thenMapToTokens { nodes -> listOf(LeafNodeToken("while (")).plus(kotlinScanner.scanNodes(nodes, ScannerState.STATEMENT)).plus(LeafNodeToken(")")) }
        nodeOfType(KtTokens.RPAR)
        zeroOrMore { anyNode() } thenMapToTokens { nodes -> kotlinScanner.scanNodes(nodes, ScannerState.STATEMENT) }
        end()
    }
