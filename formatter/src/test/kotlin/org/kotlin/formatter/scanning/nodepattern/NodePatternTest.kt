...
class NodePatternTest {
    ...
    fun `executes actions after nested groups (exactlyOne)`() {
        ...
        val subject =
            nodePattern {
                exactlyOne {
                    nodeOfType(KtTokens.CLASS_KEYWORD)
                    exactlyOne { nodeOfType(KtTokens.IDENTIFIER) }
                } thenMapToTokens {
                    accumulatedNodes.addAll(it)
                    listOf()
                }
    ...
    fun `executes actions after nested groups (zeroOrOne)`() {
        ...
        val subject =
            nodePattern {
                exactlyOne {
                    nodeOfType(KtTokens.CLASS_KEYWORD)
                    zeroOrOne { nodeOfType(KtTokens.IDENTIFIER) }
                } thenMapToTokens {
                    accumulatedNodes.addAll(it)
                    listOf()
                }
    ...
    fun `accepts zero matching tokens with Kleene star`() {
        ...
        val subject =
            nodePattern {
                zeroOrMore { nodeOfType(KtTokens.CLASS_KEYWORD) } thenMapToTokens {
                    accumulatedNodes.addAll(it)
                    listOf()
                }
                end()
    ...
    fun `accepts two matching tokens with Kleene star (frugal mode)`() {
        ...
        val subject =
            nodePattern {
                zeroOrMoreFrugal { nodeOfType(KtTokens.CLASS_KEYWORD) } thenMapToTokens {
                    accumulatedNodes = it
                    listOf()
                }
                end()
    ...
    fun `accepts as many matching nodes as possible on Kleene star`() {
        ...
        val subject =
            nodePattern {
                zeroOrMore { nodeOfType(KtTokens.IDENTIFIER) } thenMapToTokens {
                    accumulatedNodes = it
                    listOf()
                }
                zeroOrMore { anyNode() }
    ...
    fun `accepts as few matching nodes as possible on frugal Kleene star`() {
        ...
        val subject =
            nodePattern {
                zeroOrMoreFrugal { anyNode() } thenMapToTokens {
                    accumulatedNodes = it
                    listOf()
                }
    ...
    fun `accepts one matching token with one plus Kleene star`() {
        ...
        val subject =
            nodePattern {
                oneOrMore { nodeOfType(KtTokens.CLASS_KEYWORD) } thenMapToTokens {
                    accumulatedNodes.addAll(it)
                    listOf()
                }
                end()
    ...
    fun `accepts as many matching nodes as possible on one plus Kleene star`() {
        ...
        val subject =
            nodePattern {
                oneOrMore { nodeOfType(KtTokens.IDENTIFIER) } thenMapToTokens {
                    accumulatedNodes = it
                    listOf()
                }
                zeroOrMore { anyNode() }
    ...
    fun `accepts as few matching nodes as possible on frugal one plus Kleene star`() {
        ...
        val subject =
            nodePattern {
                oneOrMoreFrugal { anyNode() } thenMapToTokens {
                    accumulatedNodes = it
                    listOf()
                }
                zeroOrMore { nodeOfType(KtTokens.IDENTIFIER) }
    ...
    fun `accepts first branch of an either-or block`() {
        ...
        val subject =
            nodePattern {
                either { nodeOfType(KtTokens.VAL_KEYWORD) } or {
    ...
    fun `allows additional processing of nodes on a given group`() {
        ...
        val subject =
            nodePattern {
                exactlyOne {
                    oneOrMore { anyNode() thenMapToTokens { listOf(LeafNodeToken("a")) } }
                } thenMapTokens {
                    tokens = it
                    listOf()
                }
    ...
    fun `does not repeatedly push new empty token lists to the token stack`() {
        val subject =
            nodePattern {
                ...
                exactlyOne {
                    oneOrMore { anyNode() thenMapToTokens { listOf(LeafNodeToken("b")) } }
                } thenMapTokens { listOf(it.first()) }
                end()
            }
        val element = LeafPsiElement(KtTokens.IDENTIFIER, "anyElement")
